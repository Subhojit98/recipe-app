...
import empty from '@/assets/icons8-empty-box-50.png'
const SavedRecipes = () => {
    const { openSaveLater, setOpenFoodTab, setFoodCardDetails } = useContext<Context>(GlobalContext)
    const { getItem, setItem } = useLocalStorage("savedRecipes")
    ...
    return (
        <>
            <div className={`w-full min-h-screen ${openSaveLater} top-0 fixed z-50 bg-white dark:bg-[#1c1b22] overflow-y-auto`}>
                <Navbar />
                    ...
                    <div className="w-[85%] h-full grid grid-cols-2 mobileScreen:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 gap-y-3">
                                    ...
                                    <Card className="w-full h-full rounded-lg">
                                        ...
                                        <div className="flex flex-col justify-between h-[30%]">
                                            <CardHeader className="mt-2 ml-3">
                                                <CardTitle className="text-sm md:text-base">{data?.recipe.label}</CardTitle>
                                            </CardHeader>
                        ...
                        }
                        {
                            !isStoredData && <div className="w-full">
                                <h1 className="text-2xl font-bold">No Saved Recipes</h1>
                            </div>
                        }
                    </div>
                </div>
